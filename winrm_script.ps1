...
function Confirm-AdminPrivileges {
    if (-not (Test-AdminRights)) {
        ...
        Start-Process powershell.exe -ArgumentList "-ExecutionPolicy Bypass -File `"$scriptPath`" $choice" -Verb RunAs
        exit
    }
}
...
switch ($choice) {
    ...
    2 {
        ...
        try {
            ...
            # Create a temporary file to store WinRM details
            $tempFile = [IO.Path]::GetTempFileName()

            # Output details to the console and save to the temporary file
            PrintWinRMDetails -details $details -filePath $tempFile

            # Print the temporary file path and ask the user to save it
            Write-Output "WinRM details have been saved to $tempFile. Keep this information safe!"
            $saveToFile = Read-Host 'Would you like to save these details to another file? (yes/no)'
